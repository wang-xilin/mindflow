...
| **Evaluation benchmarks**  | R2R, REVERIE, SOON, VLN-CE, RxR-CE     | 各种 real-world manipulation tasks              |
...
| [[2304-ETPNav\|ETPNav]] | VLN | 无 VLM（task-specific Transformer） | Hybrid（high-level waypoint + low-level continuous） | Online topological map + waypoint prediction | VLN-CE/RxR-CE supervised | Hierarchical（transformer planner + heuristic controller） | 中 | Sim（Habitat） |
