...
- DAgger 风格: interactive demonstrator $*$ 给出每步教师节点
  - VLN-CE: $a_t^*$ = geodesic 距离最终目标最近的 ghost
...
5. **结果暴力**: VLN-CE +11 SR/SPL, RxR-CE +26 SR——这种量级的提升不是堆 trick 能堆出来的, 说明 topological global planning 抓到了真问题
...
6. **$\gamma$ 阈值的环境无关性未被检验**: Tab. 6 只在 VLN-CE 上扫了 $\gamma$, RxR-CE (路径更长、室内布局更复杂) 是否最优 $\gamma$ 仍是 0.5 不清楚
