...
针对每个任务单独微调。每步以概率 $p$ follow expert action（global planner 给的 optimal node），$1-p$ sample 自己的 policy。VLN-CE 按 SR+SPL 选 ckpt，RxR-CE 按 nDTW+SDTW。
...
$$
R_{VLN-CE} = \mathbb{I}(d_{final} < 1.5) + \text{SPL} - d_{final}/6
$$
...
4. **比较对象缺一类**：对比表没有列入纯 LVLM-based 的 [[2506-VLNR1|VLN-R1]] / [[2507-StreamVLN|StreamVLN]] 在 VLN-CE 的最新数字，无法看清 graph-based vs LVLM-based 在 2025 末的真实差距。考虑到论文 framing 主要在攻击 LVLM 路线，这个 omission 比较显眼。
